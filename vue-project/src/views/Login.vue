<template>
  <div class="p-20">
    <Form>
      ...
      <p v-if="error" class="bg-red-500 text-white p-3 mb-2 mt-2">
        {{ error }}
      </p>
      <Input
        label-name="Username"
...
const handleLogin = async () => {
  ...
  if (!userData.username) {
    (validationErrors.username = "Username is required"), (isValid = false);
  }
  ...
  if (isValid) {
    const res = await tryLogin(
      import.meta.env.VITE_FLASK_API_BASEURL + "/api/auth/login",
      userData
    );
